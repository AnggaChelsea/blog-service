...
class ProjectController {
    ...
    static async getPromo(req, res) {
        try {
            const isPromodata = await projectModel.findOne({isPromo: true})
            console.log(isPromodata)
            if (isPromodata) {
                res.status(200).json({message: 'succes', data: isPromodata})
            } else {
    ...
    static async getDataPaginate(req, res) {
        try {
            ...
            if (hargaTinggi != null) {
                ...
                const results = await projectsModel.find({ $or: [{ name: regex }, { description: regex }] });
                const startIndex = (page - 1) * pageinate;
                const endIndex = startIndex + pageinate;
